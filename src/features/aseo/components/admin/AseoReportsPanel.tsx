...
export const AseoReportsPanel = () => {
    ...
    const terminals = [...new Set(records.map((r: any) => r.terminal_code))];

    const reportData = {
        totalRecords: filteredRecords.length,
        byType: filteredRecords.reduce((acc: Record<string, number>, r: any) => {
            acc[r.cleaning_type] = (acc[r.cleaning_type] || 0) + 1;
            return acc;
        }, {}),
        byTerminal: filteredRecords.reduce((acc: Record<string, number>, r: any) => {
            acc[r.terminal_code] = (acc[r.terminal_code] || 0) + 1;
            return acc;
        }, {}),
        byCleaner: filteredRecords.reduce((acc: Record<string, number>, r: any) => {
            acc[r.cleaner_name] = (acc[r.cleaner_name] || 0) + 1;
            return acc;
        }, {}),
        graffitiRemoved: filteredRecords.filter((r: any) => r.graffiti_removed).length,
        stickersRemoved: filteredRecords.filter((r: any) => r.stickers_removed).length,
    };
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    {/* Date filters and selects - same as before */}
                    <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
                    ...
                    <div className="space-y-3">
                        {Object.entries(reportData.byTerminal).sort((a: any, b: any) => b[1] - a[1]).slice(0, 5).map(([terminal, count]) => (
