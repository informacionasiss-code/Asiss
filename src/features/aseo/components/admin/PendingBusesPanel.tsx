...
export const PendingBusesPanel = () => {
    const [searchPpu, setSearchPpu] = useState('');

    // TODO: Integrate with Estado de Flota API
    const mockPendingBuses = [
        { ppu: 'ABCD12', terminal: 'EL_ROBLE', lastCleaning: '2025-12-20', priority: 'HIGH' },
        { ppu: 'EFGH34', terminal: 'CERRILLOS', lastCleaning: '2025-12-22', priority: 'MEDIUM' },
        { ppu: 'IJKL56', terminal: 'EL_ROBLE', lastCleaning: '2025-12-23', priority: 'LOW' },
    ];

    const filteredBuses = mockPendingBuses.filter(b =>
        b.ppu.toLowerCase().includes(searchPpu.toLowerCase())
    );
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-600 mt-1">
                            {filteredBuses.length} buses requieren atención
                        </p>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    <div className="relative">
                        ...
                        <input
                            ...
                            value={searchPpu}
                            onChange={(e) => setSearchPpu(e.target.value)}
                            className="w-full pl-12 pr-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none text-slate-900 font-medium"
                        />
                    </div>
                </div>
                ...
                {filteredBuses.length === 0 && (
                    <div className="text-center py-12 text-slate-400">
                        <Icon name="check-circle" size={50} className="mx-auto mb-4 opacity-30" />
                        <p className="text-lg font-semibold">No hay buses pendientes</p>
                        <p className="text-sm">Todos los buses están al día con su limpieza</p>
                    </div>
                )}
            </div>
